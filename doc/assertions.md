...
| Temporal SVA (sequences, `##`, `\|->`) | `assert property (a ##1 b);` | Not supported (error) |
...
Only **single-cycle, non-temporal** properties are synthesized. The
yosys-slang frontend parses the assertion property AST and accepts
the pattern:
...
If the property contains sequences (`##`), implications (`|->`),
repetition (`[*N]`), or other temporal operators, yosys-slang reports
`SVAUnsupported` and compilation fails.
...
| `SimpleAssertionExpr.expr` | `expr` | `$check` A port |
