...
#
# Static hard blocks (6 clock regions total, confirmed via device query):
...
#
# Static logic (~19K LUTs) fits comfortably in these 6 CRs (~81K LUT capacity).
# RP gets 122/128 CRs ≈ 95% of the device.

create_pblock rp_emu_top
add_cells_to_pblock [get_pblocks rp_emu_top] [get_cells u_emu_top]

# SLR0: all except X4Y3 (emu_refclk IBUFDS)
resize_pblock [get_pblocks rp_emu_top] -add {CLOCKREGION_X0Y0:CLOCKREGION_X3Y3}
