...
struct DpiBridgePass : public Pass {
    ...
    void write_c_header(const std::vector<DpiFunction> &functions, const std::string &path) {
        std::ofstream ofs(path);
        if (!ofs.is_open()) {
            log_error("Cannot open C header output file: %s\n", path.c_str());
            return;
        }

        // Extract module name from path (remove directory and extension)
        std::string header_name = path;
        ...
        ofs << "//\n";
        ofs << "// DPI function prototypes for host-side implementation.\n";
        ofs << "// Implement these functions in your application.\n";
        ofs << "\n";
        ofs << "#ifndef " << guard << "\n";
        ofs << "#define " << guard << "\n";
        ofs << "\n";
        ...
        // Function ID defines
        ofs << "// DPI function IDs\n";
        for (const auto &func : functions) {
            std::string upper_name = func.name;
            for (auto &c : upper_name) c = toupper(c);
            ofs << "#define DPI_FUNC_" << upper_name << " " << func.func_id << "\n";
        }
        ofs << "\n";

        // Number of functions
        ofs << "// Number of DPI functions\n";
        ofs << "#define DPI_N_FUNCS " << functions.size() << "\n";
        ofs << "\n";

        // Function prototypes
        ofs << "// DPI function prototypes - implement these in your application\n";
        for (const auto &func : functions) {
            // Return type
